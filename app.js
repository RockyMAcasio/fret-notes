...
let recapOffset = 0;
const progressLabels                           = { Learning: 'Starting', Practicing: 'Getting smoother', Comfortable: 'Playing comfortably' };
const categoryLabels                                 = { Songs: 'Songs', Scales: 'Scales', Technique: 'Technique / accuracy', Theory: 'Fretboard / theory knowledge', Miscellaneous: 'Miscellaneous', Uncategorized: 'Uncategorized' };
const categories             = ['Scales', 'Songs', 'Technique', 'Theory', 'Miscellaneous'];
const nameFields                                                           = {
...
const form = get                 ('session-form');
const categoryInput = get                   ('category');
const songInput = get                  ('song');
const historyCategory = get                   ('history-category');
const historySearch = get                  ('history-search');
const dateInput = get                  ('date');
const minutesInput = get                  ('minutes');
const progressInput = get                   ('progress');
const notesInput = get                     ('notes');
...
function currentStreak(records           , today        )         {
...
function weekSummary(records           , start        ) {
...
try {
  ...
  get                 ('goal-form').querySelector('button') .disabled = true;
  get                   ('clear-goals').disabled = true;
}
function saveGoals(minutes               , days               )       {
...
function fillGoalInputs()       {
  ...
  get                  ('goal-minutes').value = goal?.minutes == null ? '' : String(goal.minutes);
  get                  ('goal-days').value = goal?.days == null ? '' : String(goal.days);
}
function validateInput(input         , allowUncategorized = false)               {
  if (!input || typeof input !== 'object') throw new Error('Please enter a session.');
  const data = input                           ;
  if (!categories.includes(data.category            ) && !(allowUncategorized && data.category === 'Uncategorized')) throw new Error('Choose what you worked on.');
  ...
  if (!['Learning', 'Practicing', 'Comfortable'].includes(data.progress          )) throw new Error('Choose a progress level.');
  if (typeof data.notes !== 'string' || data.notes.length > 1000) throw new Error('Keep your notes under 1,000 characters.');
  return { song: data.song.trim(), category: data.category                  , minutes: data.minutes, date: data.date, progress: data.progress            , notes: data.notes.trim() };
}
function setMessage(message        )       { get('message').textContent = message; }
let sessions            = [];
try {
...
function createSession(input         )          {
  ...
  const session          = { ...validateInput(input), id: crypto.randomUUID(), createdAt: Date.now() };
  persist([...sessions, session]);
  setMessage(`Saved ${session.minutes} minutes of ${session.song}. Nice work.`);
  return session;
}
function updateCategory(id        , category         )       {
  if (!categories.includes(category            )) throw new Error('Choose a valid practice category.');
  const session = sessions.find(item => item.id === id);
  if (!session) throw new Error('This session could not be found.');
  persist(sessions.map(item => item.id === id ? { ...item, category: category             } : item));
  setMessage(`Updated the category for ${session.song}.`);
}
function updateNameField()       {
  const selected = categories.includes(categoryInput.value            );
  const config = selected ? nameFields[categoryInput.value            ] : { label: 'Practice name or topic', placeholder: 'Choose a category first' };
...
function renderMomentum()       {
  ...
  const addGoalProgress = (label        , actual        , target        , unit        )       => {
...
function renderRecap()       {
  ...
  const end = moveDay(start, 6);
  const summary = weekSummary(sessions, start);
  get                   ('previous-week').disabled = start <= earliestWeek;
  get                   ('next-week').disabled = recapOffset >= 0;
  get                   ('this-week').disabled = recapOffset === 0;
  const formatDate = (key        ) => new Date(key + 'T12:00:00').toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' });
...
function renderHistory(sorted = [...sessions].sort((a, b) => b.date.localeCompare(a.date) || b.createdAt - a.createdAt))       {
  ...
  get                   ('clear-filters').hidden = !filtering;
  ...
  filtered.forEach(session => {
    ...
    const categorySelect = element('select', 'session-category')                     ;
    ...
    if (session.category === 'Uncategorized') {
      const option = element('option', '', 'Uncategorized — choose a category')                     ;
      option.value = 'Uncategorized'; option.disabled = true; categorySelect.append(option);
    }
    categories.forEach(category => {
      const option = element('option', '', categoryLabels[category])                     ;
    ...
    categorySelect.addEventListener('change', () => {
      ...
      } catch (error) { categorySelect.value = session.category; setMessage((error         ).message); }
    });
...
get('goal-form').addEventListener('submit', event => {
  ...
  const minutes = get                  ('goal-minutes').value.trim();
  const days = get                  ('goal-days').value.trim();
  try {
  ...
  } catch (error) { get('goal-message').textContent = (error         ).message; }
});
get('clear-goals').addEventListener('click', () => {
  try { saveGoals(null, null); fillGoalInputs(); }
  catch (error) { get('goal-message').textContent = (error         ).message; }
});
get('previous-week').addEventListener('click', () => { if (!get                   ('previous-week').disabled) { recapOffset--; renderRecap(); } });
